...
class Logger(commands.Cog, name="Logger"):
    ...
    @staticmethod
    async def sqlMusicLoggerGet():
        with sqlite3.connect("users.db") as db:
            cursor = db.cursor()
            cursor.execute("SELECT * FROM logs WHERE id == 2")
            db.commit()
            veri = cursor.fetchone()
            return veri

    @staticmethod
    async def sqlMusicErrorGet():
        with sqlite3.connect("users.db") as db:
            cursor = db.cursor()
            cursor.execute("SELECT * FROM logs WHERE id == 3")
            db.commit()
            veri = cursor.fetchone()
            return veri

    @staticmethod
    async def sqlMusicLogger():
        with sqlite3.connect("users.db") as db:
            cursor = db.cursor()
            cursor.execute("UPDATE logs SET count = count + 1 WHERE id == 2")
            db.commit()

    @staticmethod
    async def sqlMusicError():
        with sqlite3.connect("users.db") as db:
            cursor = db.cursor()
            cursor.execute("UPDATE logs SET count = count + 1 WHERE id == 3")
            db.commit()
