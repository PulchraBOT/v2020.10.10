...
class Oyun(commands.Cog, name="Oyun"):
    ...
    async def slot(self, msg, bahis: int=None):
        ...
        if veri:
            ...
            msj = f"""{user.mention}, `{bahis:,d}₺` bastın bakalım ne gelicek
            ——————
            ...
            if sonuc[0] == sonuc[1] and sonuc[0] == sonuc[2]:
                ...
                emb = discord.Embed(color=Yeşil, title=f"Hepsini tutturdun `{bahis:,d}₺` Kazandın!")
                emb.set_author(name=user.name, icon_url=user.avatar_url)
                emb.set_footer(text=Footer)
                ...
                await Economy.sqlUpdate(user, balance)
                emb = discord.Embed(color=Yeşil, title=f"İki tane tutturdun `{bahis:,d}₺` Kazandın!")
                emb.set_author(name=user.name, icon_url=user.avatar_url)
                emb.set_footer(text=Footer)
                return await embed.edit(embed=emb)
            else:
                emb = discord.Embed(color=Kırmızı, title=f"`-{bahis:,d}₺` Kaybettin")
                emb.set_author(name=user.name, icon_url=user.avatar_url)
                emb.set_footer(text=Footer)
                await embed.edit(embed=emb)
        else:
